...
type Toast = { id: number; type: ToastType; message: string; onClick?: () => void };

const ToastContext = createContext<(type: ToastType, message: string, onClick?: () => void) => void>(() => {});
      ...
      <div className="fixed bottom-6 right-6 flex flex-col gap-2.5 z-50">
        {toasts.map((t) => (
          <div
            key={t.id}
            onClick={t.onClick}
            className={`min-w-[260px] max-w-[320px] bg-ink text-paper-raised text-[13.5px] leading-snug rounded-md px-4 py-3 shadow-lg border-l-4 ${borderColor[t.type]} animate-[slidein_0.35s_ease-out] ${t.onClick ? "cursor-pointer" : ""}`}
          >
            {t.message}
          </div>
        ))}
